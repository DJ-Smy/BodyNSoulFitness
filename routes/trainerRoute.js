...
router.post("/get-trainer-info-by-user-id", authMiddleware, async(req, res) => {
    try {
      ...
      res.status(500)
      .send({ message: "Error getting trainer info", success: false, error: error });
    }
  })
...
router.post("/update-trainer-profile", authMiddleware, async(req, res) => {
    try {
      const trainer = await Trainer.findOneAndUpdate({ userId: req.body.userId },
        req.body);
      res.status(200).send({
        success: true,
      ...
      res.status(500)
      .send({ message: "Error updating trainer info", success: false, error: error });
    }
  })
